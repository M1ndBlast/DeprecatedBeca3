...
public class nuevaTarjeta extends AppCompatActivity {
    ...
    private class escribir extends AsyncTask<Integer, Void, Integer> {
            ...
            for (int i = 0; i < 4; i++) {
                if (params[0].intValue() != 0 || i != 0) {
                    ...
                    if (nuevaTarjeta.this.cual == 0) {
                        nuevaTarjeta.this.algo.writeBlock(params[0].intValue(), i, util.hexStringToByteArray(Clave.dumpMetro[params[0].intValue()][i]), util.hexStringToByteArray("FFFFFFFFFFFF"), true);
                    } else {
                        nuevaTarjeta.this.algo.writeBlock(params[0].intValue(), i, util.hexStringToByteArray(Clave.dumpSuburbano[params[0].intValue()][i]), util.hexStringToByteArray("FFFFFFFFFFFF"), true);
                    }
                }
            }
            return params[0];
        ...
        public void onPostExecute(Integer returno) {
            if (returno.intValue() + 1 <= 15) {
                ...
                nuevaTarjeta.this.pb.setProgress((returno2.intValue() * 100) / 15);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView((int) R.layout.activity_nueva_tarjeta);
        setTitle("ESCRIBIENDO TARJETA");
        this.lblEstado = (TextView) findViewById(R.id.lblEstadoEscritura);
